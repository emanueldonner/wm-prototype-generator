...
import{i as t,s as l,y as e}from"../../lit-element-a22611a3.js";const a=[t`
...
`];class o extends l{get _controls(){return this.renderRoot?.querySelector(".controls")??null}static properties={center:{type:String},id:{type:String},geocoder:{type:String},_mapLoaded:{type:Boolean,attribute:!1},zoom:{type:Number},_markers:{type:Array,attribute:!1}};static styles=[a,r];constructor(){super(),this.center="16.35649860253831, 48.2110270434633",this.id="",this.geocoder="false",this._mapLoaded=!1,this.zoom=16,this.bounds=[],this._markers={}}_getMarkers(){for(let t=0;t<this.querySelectorAll("wm-mapmarker").length;t++){const l=this.querySelectorAll("wm-mapmarker")[t];this.addMarker(`marker_${t}`,l.getAttribute("lng"),l.getAttribute("lat"),l.getAttribute("label"),!1,l.innerHTML)}this.resetBounds()}resetBounds(){Object.keys(this.bounds).length&&this.map.fitBounds(this.bounds,{padding:50})}_coordsToArray(t){return t.replaceAll(" ","").split(",")}_addGeoCoder(){if("true"===this.geocoder){const t=new window.MaplibreGeocoder({forwardGeocode:async t=>{const l=await fetch(`https://geocoder.maptoolkit.net/search?q=${t.query}&language=${t.language[0]}&countrycodes=${t.countries}&api_key=wien`);return{features:(await l.json()).map((t=>({type:"Feature",geometry:{type:"Point",coordinates:[t.lon,t.lat]},place_type:["place"],place_name:t.display_name,text:t.type,properties:t,center:[t.lon,t.lat]})))}}},{showResultsWhileTyping:!0,showResultMarkers:!1,maplibregl,countries:"at",language:"de",marker:!1});t.addTo(this.map),t.on("result",(t=>{this.addMarker(t.result.properties.place_id,t.result.center[0],t.result.center[1],t.result.place_name,!0)}))}}_initMTK(){window.MTK.init({apiKey:"wien"}).createMap(this.shadowRoot.querySelector(`#map_${this.id}`),{map:{location:{center:this._coordsToArray(this.center),zoom:this.zoom,fitBoundsOptions:{padding:50}},mapType:"wien-ther_stadtplan_wien"}},(t=>{console.log("inittt"),this.map=t.gl,this.map.scrollZoom.disable(),this.bounds=new window.maplibregl.LngLatBounds,this._getMarkers(t),this._addEvents(),this._addGeoCoder(),this._mapLoaded=!0,this.dispatchEvent(new CustomEvent("mapReady",{bubbles:!0,composed:!0,detail:{map:this.map}}))}))}_addEvents(){this.map.on("moveend",(t=>{this.dispatchEvent(new CustomEvent("moveend",{bubbles:!0,composed:!0,detail:{map:this.map}}))}))}_setPopupContent(t,l,e,a,r,o,i){const c=document.createElement("div");if(c.innerHTML=`<h3>${a}</h3>\n    ${i||""}\n    ${o?`<p>lng: ${l}<br>lat: ${e}</p>`:""}\n    `,o){const l=document.createElement("button");l.textContent="Löschen",l.addEventListener("click",(l=>{console.log(r),r.remove(),delete this._markers[t],this.dispatchEvent(new CustomEvent("markerRemoved",{bubbles:!0,composed:!0,detail:{markers:this._markers}}))})),c.appendChild(l)}return c}deleteAll(){this.bounds=new window.maplibregl.LngLatBounds;for(const t in this._markers)this._markers[t].marker.remove()}addMarker(t,l,e,a,r=!1,o,i){const c=new window.maplibregl.Marker({draggable:r}),p=this._setPopupContent(t,l,e,a,c,r,o),s=new window.maplibregl.Popup({offset:25}).setDOMContent(p);c.setLngLat([l,e]).setPopup(s).addTo(this.map);c.on("dragend",(()=>{const l=c.getLngLat(),e=this._setPopupContent(t,l.lng,l.lat,"Aktualisierter Pin",c,r);s.setDOMContent(e)})),this._markers[t]={lng:l,lat:e,label:a,marker:c},this.bounds.extend(new window.maplibregl.LngLat(l,e)),this.dispatchEvent(new CustomEvent("markerAdded",{bubbles:!0,composed:!0,detail:{marker:c,markers:this._markers}}))}_embedScript(t,l=!1){const e=document.createElement("script");return l&&(e.onload=this._onLoad.bind(this)),e.src=t,e}_onLoad(t){this._initMTK(),t.target.onload=null}reRender(){this.map.resize(),this.resetBounds()}render(){return e`
      ${"true"===this.geocoder?e`<div class="controls">
      ...
      ${this._embedScript("https://static.maptoolkit.net/mtk/v10.1.3/mtk.js",!0)}
      ${this._embedScript("https://unpkg.com/@maplibre/maplibre-gl-geocoder@1.2.0/dist/maplibre-gl-geocoder.min.js")}
    `}}customElements.define("wm-map",o);const i="wm-map";export{o as Map,i as tagName};
